const firebaseConfig = {
  ...
  var pendingloan = firebase.database().ref().child("loan");
  ...
  pendingloan.once("value").then(function (snapshot) {
    ...
    if (snapshot.exists()) {
      ...
      var aa = 'hhh';
        ...
        content += "<td style='text-align:center'>" + '<button type="button" class="bt" onclick=edit("' + data.key + '")><span class="layer"></span>Details</button>' +
  ...
  function edit(key) {
    ...
    ref5.on("value", function (snapshot4) {
      ...
      if (status == "Active") {
         document.getElementById("status").style.color = 'rgb(12, 182, 6)'
      }
        ...
        document.getElementById("status").style.color = 'red'
          ...
          document.getElementById("status").style.color = 'rgb(230, 175, 35)'
      }
