...
var pendingloan = firebase.database().ref().child("deposit").orderByChild('status').equalTo('Pending');
...
function edit(key) {
    ...
    ref5.on("value", function (snapshot4) {
        if (snapshot4.exists()) {
            ...
            method = snapshot4.val().transaction_name;
...
function save2() {
    ...
    if (didConfirm == true) {
        ...
        var active_dep = firebase.database().ref().child("deposit2").child(uid).orderByChild('current_time').equalTo(current_time);
        ...
        active_dep.once("value").then(function (snapshot) {
            ...
            database.ref('user-profile').child(uid).set({
                ...
                "balance": parseInt(balance2) + parseInt(amount),
                'city': city2,
                'email': email2,
            ...
            database.ref('deposit2').child(uid).child(key22).set({
            ...
            database.ref('deposit').child(ke).set({
                ...
                window.location.href = "pending-deposit.html";
            });
        });
